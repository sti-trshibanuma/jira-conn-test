...
// 管理番号 B17038 To
				...
				BL_HR_PY_03_R28 blHrPy03R28 =BL_HR_PY_03_R28.GetInstance();  
				//対象年月Toが対象年月From+12ヶ月より大きい時は、対象年月に12ヶ月を足して対象年月Toにセットする
				if (DateTime.Parse(condition.DateResvYmFrom+"/01").AddMonths(11) < DateTime.Parse(condition.DateResvYmTo+"/01"))
				{
					condition.DateResvYmTo = (DateTime.Parse(condition.DateResvYmFrom+"/01").AddMonths(11)).ToString().Substring(0,7);
				}
				...
				{
					//データ取得
					detailDt = blHrPy03R28.Select(commonData, condition, false);
				}
...
// 管理番号K27228 To
				...
				{
					//帳票の定義設定
					Infocom.Allegro.HR.rpt.HR_PY_03_R28	rpt01  = new Infocom.Allegro.HR.rpt.HR_PY_03_R28();

					// プロパティによるヘッダ情報の設定
					rpt01.commonData = commonData;
					rpt01.ReportID = "(HR_PY_03_R28)";
